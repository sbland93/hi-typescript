...
class Block {
    ...
    static validateStructure = (aBlock: Block) : boolean => typeof aBlock.index === "number" && typeof aBlock.hash === "string" && 
...
const genesisBlock:Block= new Block(0, "20202002", "", "Hello", 123456);
...
const isBlockValid = (candidateBlock: Block, previousBlock: Block) : boolean => {
    if(!Block.validateStructure(candidateBlock)) return false;
        ...
        return false;
    } else if(getHashforBlock(candidateBlock) !== candidateBlock.previousHash) {
        return false;
    } else {
        return true;
    }
